...
extern int cy_set_device_info(struct cy_device_info gn_dev_info);
#if FACTORY_COMPATIBLE_BY_DAICHENG
u8 vendor_id_template = 0;
#endif
int fts_register_hardware_info(struct i2c_client *client)
{
    u8 fw_ver = 0;
    u8 vendor_id = 0;
...
#if FTS_AUTO_LIC_UPGRADE_EN
    ret = fts_i2c_read_reg(client, FTS_REG_LIC_VER, &lcd_cfg);
    if (ret < 0) {
        return ret;
    }
#endif
    ...
    case FTS_VENDOR_ID:
        #if FTS_AUTO_LIC_UPGRADE_EN
        snprintf(cy_tp_hw_info.name, CY_DEVICE_NAME_LEN, "truly_ft6336u-ID[0x%x]-FW[0x%x]-LCM_CFG[0x%x]\n",
            vendor_id,fw_ver, lcd_cfg);
        #else
        snprintf(cy_tp_hw_info.name, CY_DEVICE_NAME_LEN, "truly_ft6336u-ID[0x%x]-FW[0x%x]\n",
            vendor_id,fw_ver);
        #endif
        break;
#endif
    ...
    case FTS_VENDOR_ID2:
        #if FTS_AUTO_LIC_UPGRADE_EN
        snprintf(cy_tp_hw_info.name, CY_DEVICE_NAME_LEN, "txd_ft6336u-ID[0x%x]-FW[0x%x]-LCM_CFG[0x%x]\n",
            vendor_id,fw_ver, lcd_cfg);
        #else
        snprintf(cy_tp_hw_info.name, CY_DEVICE_NAME_LEN, "txd_ft6336u-ID[0x%x]-FW[0x%x]\n",
            vendor_id,fw_ver);
        #endif
        break;
#endif
    ...
    case FTS_VENDOR_ID3:
        #if FTS_AUTO_LIC_UPGRADE_EN
        snprintf(cy_tp_hw_info.name, CY_DEVICE_NAME_LEN, "null_ft8006u-ID[0x%x]-FW[0x%x]-LCM_CFG[0x%x]\n",
            vendor_id,fw_ver, lcd_cfg);
        #else
        snprintf(cy_tp_hw_info.name, CY_DEVICE_NAME_LEN, "null_ft8006u-ID[0x%x]-FW[0x%x]\n",
            vendor_id,fw_ver);
        #endif
        break;
#endif
    default:
        FTS_ERROR("[Info]Hardware Info Get Failed.\n");
        break;
...
{
    int ret = 0;
    //Chenyee <CY_BSP_CTP> <xuel> <2018-01-03> Added For #CSW1705A-342 Begin
    int i = 0;
    //Chenyee <CY_BSP_CTP> <xuel> <2018-01-03> Added For #CSW1705A-342 End
...
#ifdef CONFIG_CY_BSP_MTK_DEVICE_CHECK
#if FTS_AUTO_LIC_UPGRADE_EN
    if((ctp_flag == 0) && (lcd_flag == 0))
#else
    if(ctp_flag == 0)
#endif
    fts_register_hardware_info(client);
#endif
//Chenyee <CY_BSP_CTP> <xuel> <2017-12-09> Modify For #CSW1705A-8 End
...
static int fts_fwupg_get_fw_file(struct fts_ts_data *ts_data)
{
    ...
    //Chenyee <CY_BSP_CTP> <daicheng> <2018-06-14> Modify For #CSW1803A-480 Begin
    #ifdef CONFIG_CY_BSP_CTP_COMPATIBLE_SUPPORT
    if ( vendor_id == 0 ){
        FTS_INFO("vendor id:%04x,compare lcm_name to change vendor id", vendor_id);
        if ( module_vendor_id == truly_ft6336_module ){
            vendor_id = FTS_VENDOR_ID;
            FTS_INFO("module_vendor_id:%04x  vendor id:%04x",module_vendor_id,vendor_id);
        }else if( module_vendor_id == txd_ft6336_module){
            vendor_id = FTS_VENDOR_ID2;
            FTS_INFO("module_vendor_id:%04x  vendor id:%04x",module_vendor_id,vendor_id);
        }
    }
    #endif
    //Chenyee <CY_BSP_CTP> <daicheng> <2018-06-14> Modify For #CSW1803A-480 end
    
    /* Chenyee <CY_BSP_CTP> <daicheng> <2018-05-24> Modify For #CSW1803-3 Begin */
    if (vendor_id == FTS_VENDOR_ID3){
    	vendor_id = FTS_VENDOR_ID2;
    	FTS_INFO("success to change vendor_3 to vendor_2 id:%04x", vendor_id);
    }
    /* Chenyee <CY_BSP_CTP> <daicheng> <2018-05-24> Modify For #CSW1803-3 End */
    if ((vendor_id != FTS_VENDOR_ID)&&(module_vendor_id == truly_ft6336_module)){
        FTS_INFO("exception:module_vendor_id:%04x  vendor id:%04x",module_vendor_id,vendor_id);
        vendor_id = FTS_VENDOR_ID;
        FTS_INFO("exception:force change vendor id to:%04x",vendor_id);
    }
    if ((vendor_id != FTS_VENDOR_ID2)&&(module_vendor_id == txd_ft6336_module)){
        FTS_INFO("exception:module_vendor_id:%04x  vendor id:%04x",module_vendor_id,vendor_id);
        vendor_id = FTS_VENDOR_ID2;
        FTS_INFO("exception:force change vendor id to:%04x",vendor_id);
    }
