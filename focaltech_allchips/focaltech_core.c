...
char fts_touch_name_truly[] = {"truly_st7701s"};
char fts_touch_name_txd[]= {"txd_st7701s"};
int module_vendor_id = 0;
#endif
//Chenyee <CY_BSP_CTP> <xuel> <2017-11-28> Added For #CSW1705A-8 Begin

extern int fts_wake_switch;
extern int fts_gesture_switch;

//Chenyee <CY_BSP_CTP> <xuel> <2017-07-12> Added For #CSW1705A-8 Begin
#ifdef CONFIG_CY_BSP_CTP_COMPATIBLE_SUPPORT
static int cy_fts_judge_touch_device(void)
{
    FTS_INFO("Touch type:%s\n", lcm_matching);
    if(strncmp(lcm_matching, fts_touch_name_truly, strlen(fts_touch_name_truly)) == 0) {
        FTS_INFO("%s matching.\n", fts_touch_name_truly);
        return 1;//truly_st7701s
    }
    else if(strncmp(lcm_matching, fts_touch_name_txd, strlen(fts_touch_name_txd)) == 0){
	    FTS_INFO("%s matching.\n", fts_touch_name_txd);
        return 2;//txd_st7701s
    }
...
//Chenyee <CY_BSP_CTP> <xuel> <2017-07-12> Added For #CSW1705A-8 Begin
#if FTS_GESTURE_EN
bool gn_halt_flag = false;
#endif
//Chenyee <CY_BSP_CTP> <xuel> <2017-07-12> Added For #CSW1705A-8 End
/*****************************************************************************
...
//Chenyee <CY_BSP_CTP> <xuel> <2017-12-09> Modify For #CSW1705A-8 Begin
    if((fts_wake_switch == 1) || (fts_gesture_switch == 1)) {
...
static int fts_read_touchdata(struct fts_ts_data *data)
{
    ...
    if (ret < 0) {
        ...
        //Chenyee <CY_BSP_CTP> <xuel> <2017-12-20> Modify For #CSW1705A-322 Begin
        fts_reset_proc(200); 
        fts_tp_state_recovery(client);
        //Chenyee <CY_BSP_CTP> <xuel> <2017-12-20> Modify For #CSW1705A-322 End
        return ret;
    }
...
void get_time_str(void)
{
    ...
    FTS_DEBUG("%04d-%02d-%02d %02d:%02d:%02d",tm.tm_year+1900,tm.tm_mon+1,tm.tm_mday,tm.tm_hour,tm.tm_min,tm.tm_sec);
}
...
static int __init tpd_driver_init(void)
{
    ...
    //Chenyee <CY_BSP_CTP> <xuel> <2017-11-28> Added For #CSW1705A-8 Begin
    #ifdef CONFIG_CY_BSP_CTP_COMPATIBLE_SUPPORT
    module_vendor_id = cy_fts_judge_touch_device();
    if(lcm_matching != NULL) {
        if(module_vendor_id == 0) {
            FTS_ERROR("Touch IC not match.\n");

            return -ENODEV;
        }else {
            FTS_INFO("Touch IC match.\n");
            kfree(lcm_matching);
        }
    }else {
        return -ENODEV;
