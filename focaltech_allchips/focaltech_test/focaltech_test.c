...
int get_channel_num(void)
{
    ...
    FTS_TEST_INFO("FACTORY_REG_CHX_NUM = 0x%x,FACTORY_REG_CHY_NUM = 0x%x \n",FACTORY_REG_CHX_NUM,FACTORY_REG_CHY_NUM);
...
static int fts_test_main_init(void)
{
    ...
    /* <Chenyee> <CY_BSP_TP> <daicheng> <20180510> add for factory test begin */
#if ((FTS_CHIP_TYPE == _FT6336U)||(FTS_CHIP_TYPE == _FT6336G))   
    len = test_data.screen_param.tx_num + 1;
#else
    len = (test_data.screen_param.tx_num + 1) * test_data.screen_param.rx_num;
#endif
    /* <Chenyee> <CY_BSP_TP> <daicheng> <20180510> add for factory test end */
...
static int fts_test_entry(char *ini_file_name)
{
    ...
    if (true == fts_test_start()) {
        FTS_TEST_SAVE_INFO("\n=======Tp test pass. \n\n");
	ret = 1;// pass modify by daicheng
    } else {
        FTS_TEST_SAVE_INFO("\n=======Tp test failure. \n\n");
	ret = -1;//failer modify by daicheng
    }

    //ret = 0; modify by daicheng
TEST_ERR:
    //Chenyee <CY_BSP_CTP> <daicheng> <2018-06-30> Modify For #CSW1803A-3 begin
    fts_reset_proc(1);
    msleep(1*1000);
    //Chenyee <CY_BSP_CTP> <daicheng> <2018-06-30> Modify For #CSW1803A-3 End
    enter_work_mode();
    fts_test_main_exit();
...
int ftxxxx_ftslibtest_sample(void)
{
    int result = 0;
    char fwname[128] = {0};
    u8 vendor_id = 0;
    ...
    if(ret < 0) {
        FTS_ERROR("[Info]Factory Check Mode Read Vendor ID(0x%x) Failed.\n", vendor_id);
    }
    ...
    switch(vendor_id)
    {
        ...
        case FTS_VENDOR_ID2:
            sprintf(fwname, "%s", "Conf_MultipleTest_txd.ini");
            fwname[25] = '\0';
            break;
#endif
...
* Return: EPERM
***********************************************************************/
ssize_t fts_test_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int result = 0;
...
* Return: char count
***********************************************************************/
ssize_t fts_test_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    char fwname[128] = {0};
    struct fts_ts_data *ts_data = fts_data;
    struct input_dev *input_dev;
