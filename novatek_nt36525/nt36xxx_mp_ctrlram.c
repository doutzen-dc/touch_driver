...
#else
#define SHORT_TEST_CSV_FILE "/mnt/sdcard/tpdata/ShortTest.csv"
#define OPEN_TEST_CSV_FILE "/mnt/sdcard/tpdata/OpenTest.csv"
#define FW_RAWDATA_CSV_FILE "/mnt/sdcard/tpdata/raw_data.csv"
#define FW_CC_CSV_FILE "/mnt/sdcard/tpdata/FWCCTest.csv"
#define NOISE_TEST_CSV_FILE "/mnt/sdcard/tpdata/NoiseTest.csv"
#endif
int nvt_test_result = 1;
...
static uint8_t *RecordResult_Short_Diff = RecordResult_Short;
static uint8_t RecordResult_Short_Base[IC_X_CFG_SIZE * IC_Y_CFG_SIZE + IC_KEY_CFG_SIZE] = {0};
...
static uint8_t *RecordResult_FW_CC_I = RecordResult_FW_CC;
static uint8_t RecordResult_FW_CC_Q[IC_X_CFG_SIZE * IC_Y_CFG_SIZE + IC_KEY_CFG_SIZE] = {0};
...
static int32_t *RawData_Short_Diff = RawData_Short;
static int32_t RawData_Short_Base[IC_X_CFG_SIZE * IC_Y_CFG_SIZE + IC_KEY_CFG_SIZE] = {0};
...
static int32_t *RawData_FW_CC_I = RawData_FW_CC;
static int32_t RawData_FW_CC_Q[IC_X_CFG_SIZE * IC_Y_CFG_SIZE + IC_KEY_CFG_SIZE] = {0};
...
static int32_t nvt_read_baseline(int32_t *xdata)
{
	...
	int32_t k = 0;
	int32_t key_idx = -1;
	int32_t xdata_btn_tmp[IC_KEY_CFG_SIZE] = {0};
#endif /* #if TOUCH_KEY_NUM > 0 */
...
#if TOUCH_KEY_NUM > 0
	for (k = 0; k < Key_Channel; k++) {
		xdata_btn_tmp[k] = xdata[y_num * x_num + k];
	}
	key_idx = -1;
...
static int32_t nvt_read_CC(int32_t *xdata)
{
	...
	for (k = 0; k < IC_KEY_CFG_SIZE; k++) {
		if (AIN_KEY[k] != 0xFF) {
			key_idx++;
			if (ts->carrier_system) {
				RawData_FW_CC_I[Y_Channel * X_Channel + AIN_KEY[k]] = (uint8_t)(xdata_btn_tmp[key_idx] & 0xFF);
				RawData_FW_CC_Q[Y_Channel * X_Channel + AIN_KEY[k]] = (uint8_t)((xdata_btn_tmp[key_idx] >> 8) & 0xFF);
			} else {
				xdata[Y_Channel * X_Channel + AIN_KEY[k]] = xdata_btn_tmp[key_idx];
			}
		}
...
static int32_t nvt_read_fw_noise(int32_t *xdata)
{
	...
	for (k = 0; k < IC_KEY_CFG_SIZE; k++) {
		if (AIN_KEY[k] != 0xFF) {
			key_idx++;
			if (ts->carrier_system) {
				RawData_Diff_Max[Y_Channel * X_Channel + AIN_KEY[k]] = (uint16_t)xdata_btn_tmp[key_idx];
				RawData_Diff_Min[Y_Channel * X_Channel + AIN_KEY[k]] = 0;
			} else {
...
static int32_t nvt_read_fw_short(int32_t *xdata)
{
	...
	if (ts->carrier_system) {
		...
		for (k = 0; k < IC_KEY_CFG_SIZE; k++) {
			if (AIN_KEY[k] != 0xFF) {
				iArrayIndex = Y_Channel * X_Channel + k;
				RawData_Short_Base[Y_Channel * X_Channel + AIN_KEY[k]] = (int16_t) (rawdata_buf[iArrayIndex * 2] + 256 * rawdata_buf[iArrayIndex * 2 + 1]);
			}
		}
#endif /* #if TOUCH_KEY_NUM > 0 */
	}
...
//Chenyee <CY_BSP_CTP> <xuel> <2018-03-17> Added For #CSW1705A-2247 Begin
	nvt_test_result = 1;
//Chenyee <CY_BSP_CTP> <xuel> <2018-03-17> Added For #CSW1705A-2247 End
...
//Chenyee <CY_BSP_CTP> <xuel> <2018-01-25> Modify For #CSW1705A-1266 Begin
		if ((TestResult_FWMutual == -1) || (TestResult_FW_CC == -1)) {
			TestResult_FW_Rawdata = -1;
			nvt_change_mode(NORMAL_MODE);
			nvt_test_result = -1;
			goto nvt_test_failed;
		}else
			TestResult_FW_Rawdata = 0;
//Chenyee <CY_BSP_CTP> <xuel> <2018-01-25> Modify For #CSW1705A-1266 End
	}
