...
#define RTPM_PRIO_TPD 0x4

#define UBL_I2C_ADDR 0x2c

//Chenyee <CY_BSP_CTP> <hezheng> <2017-10-18> modify For ID241394 Begin 
#define SENSOR_MAX_X 720	//1080
#define SENSOR_MAX_Y 1280	//2160	//1920->2160
//Chenyee <CY_BSP_CTP> <hezheng> <2017-10-18> modify For ID241394 end 

// Gionee <GN_BSP_CTP> <linzhanchi> <20170817>  modify for ID186117 begin
#define WAKEUP_GESTURE true
// Gionee <GN_BSP_CTP> <linzhanchi> <20170817>  modify for ID186117 end

//Gionee <GN_BSP_CTP> <linzhanchi> <20170825> modify for ID195979 start
#define SYNAPTICS_ESD_CHECK_EN 	0
...
#define STATUS_CODE_DISPLAY_FAILURE 0x09
...
#define SYNAPTICS_SLEEP_BEFORE_POWEROFF

//#define GN_SYNAPTICS_DEBUG_LOG

int td4310_display_abnormal = 0;
//int td4310_esd_recovery = 0;

int synaptics_log_switch = 0;

// Gionee <GN_BSP_CTP> <linzhanchi> <20170814>  add for ID163745 begin
#if GN_BSP_TP_MATCH_MODE
extern int synaptics_touch_protect;
#endif
// Gionee <GN_BSP_CTP> <linzhanchi> <20170814>  add for ID163745 end

#if GN_BSP_TP_MATCH_MODE
extern char *lcm_matching;
static char tm_synaptics_touch[] = "tm_td4310";
#endif

//Gionee <GN_BSP_CTP> <linzhanchi> <20170825> modify for ID195979 start
#if SYNAPTICS_ESD_CHECK_EN
#define SYNAPTICS_RESET_LCD_RECOVERY
#define SYNAPTICS_ESD_CHECK_CIRCLE 2000
static struct delayed_work synaptics_rmi4_esd_check_work;
static struct workqueue_struct *synaptics_rmi4_esd_check_workqueue;
static void synaptics_rmi4_esd_check_func(struct work_struct *);
static int tpd_report_flag = 0;
static unsigned int tpd_esd_flag = 0;
#ifdef SYNAPTICS_RESET_LCD_RECOVERY
...
//#ifdef CONFIG_GN_BSP_CTP_HALL_SUPPORT
#if 0
#define	HOLSTER_CLOSE	0
#define HOLSTER_OPEN	1
#define SYNAPTICS_HOLSTER_ADDR	0x1E
#define HOLSTER_OPEN_COVER		0x0
#define HOLSTER_CLOSE_COVER		0x2

struct touch_hall {
	void (*hall_callback)(int);
};

extern struct touch_hall touch_hall_call;
extern atomic_t holster_intr_protect;
static struct synaptics_rmi4_data *rmi4_holster_data = NULL;
#endif
...
// Gionee <GN_BSP_CTP> <linzhanchi> <20170803>  modify for ID180964 begin
#if 0
#ifdef CONFIG_OF_TOUCH
...
#endif
#endif
// Gionee <GN_BSP_CTP> <linzhanchi> <20170803>  modify for ID180964 end
...
#if SYNAPTICS_ESD_CHECK_EN
static ssize_t synaptics_rmi4_esd_check_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_esd_check_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
#endif
...
// Gionee <GN_BSP_LCD> <zhoucs> <20170821> modify for ID191825 begin
static struct synaptics_rmi4_data *g_rmi4_ptr = NULL;
// Gionee <GN_BSP_LCD> <zhoucs> <20170821> modify for ID191825 end

#define GN_MTK_BSP_TP_GN_FEATURE

#if defined(GN_MTK_BSP_TP_GN_FEATURE)
#include "gn_tpd_feature.h"
extern int synaptics_wake_switch;
extern int synaptics_gesture_switch;
#endif

#if defined(GN_MTK_BSP_TP_GLOVE_SUPPORT)
extern unsigned int synaptics_glove_switch;
#endif
...
#if SYNAPTICS_ESD_CHECK_EN
	__ATTR(esd_check, 0660,
			synaptics_rmi4_esd_check_show,
			synaptics_rmi4_esd_check_store),
#endif
};
//Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 start

//#ifdef CONFIG_GN_BSP_CTP_HALL_SUPPORT
#if 0
static int chip_td4310_id = 0;
static unsigned int hall_key_state = 0;
//Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 start

#define SYNAPTICS_COVER_MODE_EN		   			0x1D
/*
#define SYNAPTICS_COVER_MODE_LEFT_HIGH		    0xC4
#define SYNAPTICS_COVER_MODE_LEFT_LOW		   		0xC5
#define SYNAPTICS_COVER_MODE_RIGHT_HIGH		    0xC6
#define SYNAPTICS_COVER_MODE_RIGHT_LOW		    0xC7
#define SYNAPTICS_COVER_MODE_UP_HIGH		   		0xC8
#define SYNAPTICS_COVER_MODE_UP_LOW		   		0xC9
#define SYNAPTICS_COVER_MODE_BOTTOM_HIGH		    0xCA
#define SYNAPTICS_COVER_MODE_BOTTOM_LOW		   	0xCB
*/

//Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 start
static struct workqueue_struct *synaptics_holster_workqueue = NULL;
static struct work_struct synaptics_holster_work;

static void synaptics_holster_setting(int enable)
{
	unsigned char tp_holster_mode = 0x0;
	int retval = 0;

	if (NULL == rmi4_holster_data)
		return;
	
	if (1 == enable) {		// Close the cover
		tp_holster_mode = HOLSTER_CLOSE_COVER;
		
		if (1 != tpd_halt) {
	 		retval = synaptics_rmi4_i2c_write(rmi4_holster_data, SYNAPTICS_HOLSTER_ADDR, &tp_holster_mode, sizeof(tp_holster_mode));
			if (retval < 0)
				GN_DEBUG_ERROR("%s: Failed to write 0x1E", __func__);
		}
		
		GN_DEBUG_INFO("%s:enable holster", __func__);	
	} else {  	// Open the cover
		tp_holster_mode = HOLSTER_OPEN_COVER;
		...
		GN_DEBUG_INFO("%s: disable holster", __func__);
 	}
	
}

static void synatpics_hall_callback_work_func(struct work_struct *work)
{
		if(hall_key_state == 0)
		{
			synaptics_holster_setting(1);
		}		
		else
		{
			synaptics_holster_setting(0);
		}
}

 void synaptics_hall_key_callback(int state)
{
		GN_DEBUG_INFO("%d %s  state = %d!",__LINE__,__func__,state);

		hall_key_state = state;
		if (NULL == synaptics_holster_workqueue) {
			GN_DEBUG_INFO("create workqueue failed!");
			return;
		}	
		queue_work(synaptics_holster_workqueue, &synaptics_holster_work);
}
#endif
//Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 end
...
#if GN_BSP_TP_MATCH_MODE
//Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 start
static int synaptics_judge_touch(void)
{
		GN_DEBUG_INFO("[GNCTP] synaptics touch type:%s",lcm_matching);
        if (0 == strncmp(lcm_matching, tm_synaptics_touch, strlen(tm_synaptics_touch)))
        {
                GN_DEBUG_INFO("[GNCTP]Synaptics touch match");
                return 1;
        }
...
int synaptics_rmi4_is_updating(void)
{
    //struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(g_dev);
	struct synaptics_rmi4_data *rmi4_data = g_rmi4_ptr;
	
	if (rmi4_data->stay_update_awake || rmi4_data->staying_awake){
		printk("%s:updating FW, stay_update_awake = %d, staying_awake = %d\n", 
			  __func__, rmi4_data->stay_update_awake, rmi4_data->staying_awake);
		return 1;
	}
    
    printk("%s: no updating.\n", __func__);
...
#if SYNAPTICS_ESD_CHECK_EN
 void synaptics_rmi4_esd_check_enable(bool enable)
 {
     if (enable){
	 	
		queue_delayed_work(synaptics_rmi4_esd_check_workqueue, &synaptics_rmi4_esd_check_work, msecs_to_jiffies(SYNAPTICS_ESD_CHECK_CIRCLE));
     }
     else{
	 	
 		cancel_delayed_work_sync(&synaptics_rmi4_esd_check_work);
		
     }
 }

//Gionee <GN_BSP_CTP> <linzhanchi> <20170825> modify for ID195979 start
#ifndef SYNAPTICS_RESET_LCD_RECOVERY
 static void synaptics_rmi4_hw_reset(void)
 {
 #ifdef CONFIG_OF_TOUCH
			disable_irq_nosync(touch_irq);	// disable interrupt 
#else
			mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
#endif
  
     mt_set_gpio_mode(GPIO146_LCM_RESET, 0);
     mt_set_gpio_dir(GPIO146_LCM_RESET, GPIO_DIR_OUT);
     mt_set_gpio_out(GPIO146_LCM_RESET, 1);
     msleep(DELAY_RESET_LOW);
	
     mt_set_gpio_out(GPIO146_LCM_RESET, 0);
     msleep(DELAY_RESET_LOW);
...
#ifdef CONFIG_OF_TOUCH
	 enable_irq(touch_irq);		// enable interrupt
#else
...
#endif
 //Gionee <GN_BSP_CTP> <linzhanchi> <20170825> modify for ID195979 endd
 
static void synaptics_rmi4_esd_check_func(struct work_struct *work)
{
//Gionee <GN_BSP_CTP> <linzhanchi> <20170825> modify for ID195979 start
    //struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(g_dev);
	struct synaptics_rmi4_data *rmi4_data = g_rmi4_ptr;
    unsigned long esd_check_circle = SYNAPTICS_ESD_CHECK_CIRCLE;
    u8 bootloader_id[2] = {0, 0};
	int i = 0;
	int retval = 0;
...
//Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 start			   
    if (tpd_report_flag) {
		GN_DEBUG_ERROR("%s:Detect finger,stop esd chec", __func__);
        return;
    }
//Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 end
 //Gionee <GN_BSP_CTP> <linzhanchi> <20170825> modify for ID195979 end
    ...
    if (tpd_halt || \
        ...
        rmi4_data->stay_update_awake || \
        rmi4_data->stay_selftest_awake)
    {
        dev_err(&rmi4_data->i2c_client->dev,
			   "%s: stay awake(%d%d%d%d), skip esd check\n",
			    __func__,
               tpd_halt,
               rmi4_data->stay_awake,
               rmi4_data->stay_update_awake,
...
//Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 start
#ifdef GN_SYNAPTICS_DEBUG_LOG					
		if (0 == synaptics_log_switch) {
			TP_TRACE("esd: bootloader_id[0] = 0x%x, bootloader_id[1] = 0x%x", bootloader_id[0], bootloader_id[1]);
		} 
		else
#endif
		{
			GN_DEBUG_LOG(synaptics_log_switch, "esd: bootloader_id[0] = 0x%x, bootloader_id[1] = 0x%x", bootloader_id[0], bootloader_id[1]);
		}
//Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 stendart
        if ((bootloader_id[0] == 0x56) && (bootloader_id[1] == 0x36)) {
            break;
        }
    }
...
//Gionee <GN_BSP_CTP> <linzhanchi> <20170825> modify for ID195979 start		
	if ((i >= 3) || (tpd_esd_flag)) {
        //reset 
#ifndef SYNAPTICS_RESET_LCD_RECOVERY
        synaptics_rmi4_hw_reset();
else
		touch_reset_sig_for_lcd = 1;
#endif
        esd_check_circle = 2000;
		tpd_esd_flag = 0;
        
        dev_err(&rmi4_data->i2c_client->dev,
			   "%s: esd check abnormal, reset\n",
			   __func__);
    }
//Gionee <GN_BSP_CTP> <linzhanchi> <20170825> modify for ID195979 end

    if (!tpd_halt) {
        queue_delayed_work(synaptics_rmi4_esd_check_workqueue, \
			               &synaptics_rmi4_esd_check_work, \
			               msecs_to_jiffies(SYNAPTICS_ESD_CHECK_CIRCLE));
	}
...
// Gionee <GN_BSP_CTP> <linzhanchi> <20170817>  modify for ID186117 end
	...
	for (finger = 0; finger < fingers_supported; finger++) {
		...
		if (finger_status) {
...
#ifdef REPORT_2D_W
			input_report_abs(tpd->dev /*rmi4_data->input_dev*/,
					ABS_MT_TOUCH_MAJOR, max(wx, wy)*10);
			input_report_abs(tpd->dev /*rmi4_data->input_dev*/,
			...
			if (0 == synaptics_log_switch)
// Gionee <GN_BSP_CTP> <linzhanchi> <20170803>  modify for ID180964 begin
				TP_TRACE("down: x = %d, y = %d, size = %d, id = %d", x, y, max(wx, wy), finger);
			else
#endif
				GN_DEBUG_LOG(synaptics_log_switch, "down: x = %d, y = %d, size = %d, id = %d", x, y, max(wx, wy), finger);

			touch_count++;
		}else {
			//TP_TRACE("up: id = %d", finger);
		}
// Gionee <GN_BSP_CTP> <linzhanchi> <20170803>  modify for ID180964 end
	}
...
#ifdef GN_SYNAPTICS_DEBUG_LOG
		if (0 == synaptics_log_switch)
			TP_TRACE("release:All fingers up");
		else
#endif
			GN_DEBUG_LOG(synaptics_log_switch, "release:All fingers up");
// Gionee <GN_BSP_CTP> <linzhanchi> <20170803>  modify for ID180964 end
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->sensor_sleep && rmi4_data->enable_wakeup_gesture) {
		...
		synaptics_rmi4_gesture_func(rmi4_data,fhandler,detected_gestures[0]);
#endif
	...
	if(rmi4_data->sensor_sleep){
		dev_err(&rmi4_data->i2c_client->dev,
			"%s: the IC is suspend,ignore any touch \n",__func__);
		return 0;
	}
...
// Gionee <GN_BSP_CTP> <linzhanchi> <20170803>  modify for ID180912 begin
		/*dev_dbg(&rmi4_data->i2c_client->dev,
			"%s: Number of fingers to process = %d\n",
			__func__, fingers_to_process);*/
// Gionee <GN_BSP_CTP> <linzhanchi> <20170803>  modify for ID180912 end
	}
...
//Gionee <GN_BSP_CTP> <linzhanchi> <20170825> modify for ID195979 start
        /* display abnormal */
        if(status.status_code == STATUS_CODE_DISPLAY_FAILURE)
        {
            touch_reset_sig_for_lcd = 1;
        }
//Gionee <GN_BSP_CTP> <linzhanchi> <20170825> modify for ID195979 end
...
#if SYNAPTICS_ESD_CHECK_EN
        synaptics_rmi4_esd_check_enable(false);
        tpd_report_flag = 1;
#endif
...
#if SYNAPTICS_ESD_CHECK_EN
        synaptics_rmi4_esd_check_enable(true);
        tpd_report_flag = 0;
#endif
...
static int synaptics_rmi4_query_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	unsigned short f34_ctrl_base_addr = 0;
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			case SYNAPTICS_RMI4_F34:
				f34_ctrl_base_addr = rmi_fd.ctrl_base_addr;
				break;
			}
...
flash_prog_mode:
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,f34_ctrl_base_addr,config_id,sizeof(config_id));
	if(retval < 0)
		TPD_DEBUG("read config_id failed  err = %d \n", retval);
	else
		TPD_DEBUG("synaptics config_id = %s \n", config_id);
		
	rmi4_data->config_id = (unsigned int)config_id[3] + (unsigned int)config_id[2]*0x100;
	
#if defined(CONFIG_CY_BSP_MTK_DEVICE_CHECK)
	cy_mydev_info.cy_dev_type = CY_DEVICE_TYPE_TP;
	snprintf(cy_mydev_info.name, 50, "TCL_TD4310, fw[0x%02x%02x]", config_id[2], config_id[3]);
			//rmi4_data->sensor_max_x, rmi4_data->sensor_max_y);
	cy_set_device_info(cy_mydev_info);
#endif
...
// Gionee <GN_BSP_CTP> <linzhanchi> <20170817>  modify for ID186117 begin
#if defined(GN_MTK_BSP_TP_GESTURE_SUPPORT)
	gesture_inputdev_key_init(rmi4_data);
#endif
// Gionee <GN_BSP_CTP> <linzhanchi> <20170817>  modify for ID186117 end
...
#if defined(GN_MTK_BSP_TP_SELF_TEST)
	retval = synaptics_rmi4_f54_data_init(rmi4_data);
	if(retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
		"%s: Failed to init f54 data; retval=%d\n",
		__func__,retval);
	}
#endif
...
//Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 start
#if GN_BSP_TP_MATCH_MODE
	if (NULL != lcm_matching)
	{
		lcm_match_id = synaptics_judge_touch();
		if (0 == lcm_match_id)
		{
				synaptics_touch_protect = 1;
				GN_DEBUG_INFO("Synaptics IC has no match!");
				return -ENODEV;
		}
		else
			kfree(lcm_matching);
...
#if 0
		chip_td4310_id = 1;
		if (NULL == touch_hall_call.hall_callback)
		{
			touch_hall_call.hall_callback = synaptics_hall_key_callback;
		}
#endif
		} else {
			return -ENODEV;	
...
// Gionee <GN_BSP_CTP> <linzhanchi> <20170803>  modify for ID180964 end
	...
	rmi4_data->sensor_sleep = false;
// Gionee <GN_BSP_CTP> <linzhanchi> <20170803>  modify for ID180964 begin
	rmi4_data->irq_enabled = true; //false->true
// Gionee <GN_BSP_CTP> <linzhanchi> <20170803>  modify for ID180964 end
	rmi4_data->fingers_on_2d = false;
	...
	//Chenyee <CY_BSP_CTP> <hezheng> <2017-10-18> add For ID241394 Begin 
	dev_err(&client->dev,
				"%s: synaptics i2c addr is 0x%x\n",__func__,client->addr);
	//Chenyee <CY_BSP_CTP> <hezheng> <2017-10-18> add For ID241394 end 
...
//Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 start
//#if CONFIG_GN_BSP_CTP_HALL_SUPPORT
#if 0
		rmi4_holster_data = rmi4_data;
#endif

	ret = tpd_gesture_init();
	if (ret)
	{
		dev_err(&client->dev,
				"%s: Failed to init tpd gesture\n",
				__func__);
	}
...
//#ifdef CONFIG_GN_BSP_CTP_HALL_SUPPORT
#if 0
			INIT_WORK(&synaptics_holster_work, synatpics_hall_callback_work_func);
#endif
//Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 end
...
//Gionee <GN_BSP_CTP> <linzhanchi> <20170825> add for ID195979 start
#if SYNAPTICS_ESD_CHECK_EN
	INIT_DELAYED_WORK(&synaptics_rmi4_esd_check_work, synaptics_rmi4_esd_check_func);
	synaptics_rmi4_esd_check_workqueue = create_workqueue("synaptics_rmi4_esd_check_func");
	queue_delayed_work(synaptics_rmi4_esd_check_workqueue, \
					   &synaptics_rmi4_esd_check_work, \
					   msecs_to_jiffies(SYNAPTICS_ESD_CHECK_CIRCLE));
#endif
//Gionee <GN_BSP_CTP> <linzhanchi> <20170825> add for ID195979 end
...
#endif
{
	...
	//rmi4_data->input_dev = NULL;
#if SYNAPTICS_ESD_CHECK_EN
    destroy_workqueue(synaptics_rmi4_esd_check_workqueue);
#endif
	tpd_gesture_exit();
//Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 start
//#ifdef CONFIG_GN_BSP_CTP_HALL_SUPPORT
#if 0
		if(synaptics_holster_workqueue)
		{
			destroy_workqueue(synaptics_holster_workqueue);
		}
#endif
//Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 end
...
#if defined(GN_MTK_BSP_TP_GLOVE_SUPPORT)
void synaptics_rmi4_glove_control(bool enable)
{
	int retval;
	unsigned char offset;
	unsigned char obj_report_enable;
	struct synaptics_rmi4_fn *fhandler;
	struct synaptics_rmi4_device_info *rmi;
	struct synaptics_rmi4_f12_extra_data *extra_data;
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;
//Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 start
	if(rmi4_data == NULL) {
		GN_DEBUG_ERROR("%s  rmi4_data is null", __func__);
		return;
	}
	if (rmi4_data->stay_awake) {
		GN_DEBUG_ERROR("%s TP IC is updating firmware", __func__);
		return;
	}
	rmi = &(rmi4_data->rmi4_mod_info);
//Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 end
	...
	extra_data = (struct synaptics_rmi4_f12_extra_data *)fhandler->extra;
	extra_data->ctrl23_offset=F12_2D_CTRL23_OFFSET;
	offset = extra_data->ctrl23_offset;

	retval = synaptics_rmi4_reg_read(rmi4_data, 
			fhandler->full_addr.ctrl_base + offset,
			&obj_report_enable,sizeof(obj_report_enable));
	...
	TPD_DEBUG("%s , addr:0x%4x; glove_mode:0x%x; \n ",__func__,
			fhandler->full_addr.ctrl_base + offset,obj_report_enable );
	
	if(enable)
		obj_report_enable = obj_report_enable | 0x20;
	else
		obj_report_enable = obj_report_enable & 0xdf;
...
EXPORT_SYMBOL(synaptics_rmi4_glove_control);
...
#if defined(GN_MTK_BSP_TP_GN_FEATURE)
		synaptics_rmi4_gesture_confige_update(rmi4_data,fhandler);
#endif	
	}else {
...
#ifdef SYNAPTICS_SLEEP_BEFORE_POWEROFF
void synaptics_deep_sleep(void)
{
	struct synaptics_rmi4_data *rmi4_data = g_rmi4_ptr;
...
//#ifdef CONFIG_GN_BSP_CTP_HALL_SUPPORT
#if 0
	if (0 == atomic_read(&holster_intr_protect)) {
		if(hall_key_state == 0)
			synaptics_holster_setting(0);
	}
#endif
...
// Gionee <GN_BSP_LCD> <zhoucs> <20170821> modify for ID191825 end
	...
	if (!rmi4_data->sensor_sleep) {
		rmi4_data->touch_stopped = true;
		synaptics_rmi4_irq_enable(g_rmi4_ptr, false);
...
//Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 start
	if ((synaptics_wake_switch == 1)||(synaptics_gesture_switch == 1)){
		GN_DEBUG_INFO("%s synaptics_wake_switch enter",__func__);
		synaptics_rmi4_wakeup_gesture(rmi4_data, true);
		goto exit;
	}

#ifndef	SYNAPTICS_SLEEP_BEFORE_POWEROFF	
//#ifdef CONFIG_GN_BSP_CTP_HALL_SUPPORT
#if 0
	if (0 == atomic_read(&holster_intr_protect)) {
		if(hall_key_state == 0)
...
static void synaptics_rmi4_resume(struct device *dev)
{
 ...
 //Gionee <GN_BSP_CTP> <linzhanchi> <20170826> modify for ID197141 start
	GN_DEBUG_INFO("%s enter", __func__);
	...
	if ((synaptics_wake_switch == 1)||(synaptics_gesture_switch == 1))
	{
		TPD_DEBUG("%s synaptics_wake_switch exit\n",__func__);
...
static int tpd_local_init(void)
{
	...
	//Chenyee <CY_BSP_CTP> <hezheng> <201701018> add for ID241394 start
	if(tpd_load_status == 0) {
        TPD_DMESG("td4310 add error touch panel driver.\n");
        i2c_del_driver(&tpd_i2c_driver);
        return -1;
    }
	//Chenyee <CY_BSP_CTP> <hezheng> <201701018> add for ID241394 end
...
//#ifdef CONFIG_GN_BSP_CTP_HALL_SUPPORT
#if 0
			synaptics_holster_workqueue = create_singlethread_workqueue("fts_td4310_holster_work");
			if (synaptics_holster_workqueue == NULL) {
